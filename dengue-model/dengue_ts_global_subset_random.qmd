...
This notebook aims to validate the global/subset approach by comparing the performance of results with a random subset group attribution.
...
Assign each municipality to a random group from five groups.

```{r}
cluster_ids <- tibble(
  mun = unique(tdengue$mun),
  group = sample(x = 1:5, size = length(unique(tdengue$mun)), replace = TRUE)
) 
```
