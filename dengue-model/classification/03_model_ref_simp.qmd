...
```{r}
dc_ref_simp_split <- initial_validation_split(dc_ref_simp, prop = c(0.6, 0.2), strata = CLASSI_FIN)

dc_ref_simp_training <- training(dc_ref_simp_split)
dc_ref_simp_testing <- testing(dc_ref_simp_split)
dc_ref_simp_validating <- validation_set(dc_ref_simp_split)
```
...
rec_ref_simp_2 <- recipe(CLASSI_FIN ~ ., data = dc_ref_simp_training) |>
  ...
  step_dummy(DT_SIN_PRI_month, one_hot = TRUE) |>
  step_dummy(uf_res, one_hot = TRUE) |>
  step_integer(all_logical_predictors()) |>
  step_mutate_at(all_of(symp_cond_vars), fn = ~replace_na(., 0)) |>
  step_other(CS_GESTANT) |>
  step_dummy(CS_SEXO, CS_GESTANT, one_hot = TRUE)
```
...
ref_simp_tuning_results <- 
  ...
  workflow_map(
    "tune_race_anova",
    seed = 345,
    resamples = dc_ref_simp_validating,
    grid = 10,
    metrics = metric_set(
      accuracy, 
      roc_auc, 
